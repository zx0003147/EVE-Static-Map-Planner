...
internal data class SovereigntyLoadResult(
    val repository: SovereigntyRepository,
    val ignoredRecordCount: Int,
    val failure: Throwable? = null,
)

internal class SovereigntyRepository private constructor(
    records: Map<Int, SovereigntyRecord>,
) {
    private val recordsBySystemId = records.toMap()
    ...
    companion object {
        fun load(input: InputStream?): SovereigntyLoadResult {
            if (input == null) {
                return SovereigntyLoadResult(
                    repository = SovereigntyRepository(emptyMap()),
                    ignoredRecordCount = 0,
                    failure = IllegalStateException("Bundled sovereignty.json resource is missing"),
                )
            }
            return try {
                ...
                SovereigntyLoadResult(SovereigntyRepository(records), ignored)
            } catch (error: Throwable) {
                SovereigntyLoadResult(SovereigntyRepository(emptyMap()), 0, error)
            }
        }
    }
}
