...
private val PLANNER_SYSTEM_PROMPT = """
    ...
    When the user explicitly requests an EVE navigation send and the Mission route plus one available character are unambiguous, you must call send_mission_navigation_to_eve. Do not stop after inspecting Missions or characters, and never merely promise that a confirmation dialog will appear.
    A protected tool call waits for the real Planner confirmation decision. Base the final answer only on its returned status: report success only after a succeeded result, report cancellation after a cancelled result, and report failure after a failed result. Never say "sending now", "confirmation will appear", or an equivalent future-state claim without actually calling the protected tool.
    ...
    Reply in the language used by the user's current message whenever practical.
    Keep answers concise and state only facts supported by tool results. Do not add general EVE background or map facts from model knowledge. Do not expose Mission IDs, route IDs, overlay IDs, marker IDs, or full numeric system paths unless the user explicitly asks for them or they are essential to resolve an ambiguity.
