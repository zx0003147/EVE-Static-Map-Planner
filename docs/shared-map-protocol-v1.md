...
- CORS is disabled by default. Browser-origin access remains outside the supported product and requires an explicit
  exact-origin allowlist; wildcard origins and credentialed CORS are not supported. Desktop requests omit `Origin` and are
